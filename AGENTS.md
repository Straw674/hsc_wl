...
- **Interactive Visualization**: For all data visualizations, scripts must **both** display the image (`plt.show()`) for interactive inspection in VS Code AND save the figure (`plt.savefig()`).
  ...
  - **Bokeh Interactive Plotting Conventions**: When plotting catalog members or other coordinate-based structures on sky projections (RA/Dec) in interactive Bokeh figures, always prefer data-space glyphs (such as `p.ellipse` or `p.rect` with `width` and `height` specified in data units/degrees) over screen-space markers (such as `p.scatter` with `size` in pixels). This ensures that the markers scale and resize naturally with the astronomical image and background when zooming in or out.